 ...
 *
 * This version uses Simulated Annealing locked to the Theorem 2.2
 * structural symmetries. Instead of mutating arbitrary elements,
 * we mutate symmetric 4-tuples constrained to sum to 0 mod 4.
 *
 * This shrinks the search space tremendously and smooths the NPAF gradient.
 *
 * Compile:
 *   g++ -O3 -march=native -std=c++17 -Xpreprocessor -fopenmp \
 *       -o bin/wz_sa src/solver/wz_sa.cpp
 */
  ...
  for (int iter = 0; iter < sa.iterations; iter++) {
    ...
    if (best_cost == 0) {
      if (hall_ok(best_state.C, n, best_state.D, n))
        return true;
      // if hall fails, push cost up so it escapes
      current_cost += 50;
    }
    ...
    for (int s = 1; s < ms; s++) {
      if (left - s >= 0)
        delta_corr[s] -= curr.C[left - s] * oldC_L + curr.D[left - s] * oldD_L;
      if (left + s < n)
        delta_corr[s] -= oldC_L * curr.C[left + s] + oldD_L * curr.D[left + s];
      if (right - s >= 0)
        delta_corr[s] -=
      ...
      if (right - left == s)
        delta_corr[s] +=
            oldC_L * oldC_R + oldD_L * oldD_R; // it was subtracted twice
    }

    // Temporarily put new values
    curr.C[left] = nC_L;
    curr.D[left] = nD_L;
    curr.C[right] = nC_R;
    curr.D[right] = nD_R;
    ...
    for (int s = 1; s < ms; s++) {
      ...
      if (right - left == s)
        delta_corr[s] -= nC_L * nC_R + nD_L * nD_R; // subtracted twice addition
    }
...
bool solve_AB_SA(int n1, int ta, int tb, const int *cd_full,
  ...
  for (int iter = 0; iter < sa.iterations; iter++) {
    ...
    int old_sum_a = curr.sum_a;
    int old_sum_b = curr.sum_b;
    if (left == right) {
    ...
    for (int s = 1; s < ms; s++) {
      if (left - s >= 0)
        delta_corr[s] -= curr.A[left - s] * oldA_L + curr.B[left - s] * oldB_L;
      if (left + s < n1)
        delta_corr[s] -= oldA_L * curr.A[left + s] + oldB_L * curr.B[left + s];
      if (left != right) {
        if (right - s >= 0)
          delta_corr[s] -=
    ...
    for (int s = 1; s < ms; s++) {
      ...
      if (left != right) {
        if (right - s >= 0)
          delta_corr[s] += curr.A[right - s] * nA_R + curr.B[right - s] * nB_R;
        if (right + s < n1)
          delta_corr[s] += nA_R * curr.A[right + s] + nB_R * curr.B[right + s];
        if (right - left == s)
