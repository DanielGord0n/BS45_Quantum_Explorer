# BS45 HANDOFF archive: TOP OF MIND entries 2026-07-24 to 2026-09-14

Moved out of HANDOFF.md on 2026-09-25 to keep the live file readable. Newest first, verbatim. Older history (to 2026-07-23): HANDOFF_ARCHIVE_to_2026-07-23.md in this folder.
...
docs/plans/lever19_sweep_plan.md (commit 048109b). TRANCHE 1 via duo_run (4 taps, all echoed):
...
rule when needed; lever 20 stays GATED off n=44 until a control re-finds a known n=43. Pre-registered test in docs/research/n44_search_narrowing_research.md:
...
docs/plans/lever19_sweep_plan.md: hitless after rep 3 => disable canon fleet-wide).
...
docs/plans/lever19_sweep_plan.md (prompt updated: no front top-ups while sweep pending; advance
...
shallow window sweep (docs/research/n44_search_narrowing_research.md, design pending: walltime,
...
docs/research/n44_search_narrowing_research.md): board audit against hit-location evidence
...
accounting added to docs/briefs/paper_methods_record.md §7 — 12h lanes exhaust only tens of
...
unpriced lever, ready to queue).** (1) `docs/briefs/paper_methods_record.md` = self-contained
...
`cluster/archive/deploy/gpu_spike2.sh` (30min, h100, production budget 5e7, 6k cands, exact
...
compile `docs/briefs/paper_methods_record.md` (job IDs/timings/configs from HANDOFF+archive)
...
the DECIDING x86/AVX-512 number = `cluster/archive/deploy/place_v2_bench.sh` (30min, 1 core,
...
Recorded in `docs/research/n44_search_narrowing_research.md` lever 3; all three throughput
...
(2) `docs/briefs/kotsireas_brief.md` REWRITTEN from the stale 07-07 SA-era version: now leads
...
(Fir; 30min, 1 GPU):
```
cd ~/Projects/BS45_Quantum_Explorer && tar -cf - src/solver/gpu/fh_gpu_spike.cu cluster/archive/deploy/gpu_spike.sh results/archive/n44_cands_20k.txt | ssh dangord@fir.alliancecan.ca 'cd $SCRATCH/bs45 && tar -xvf - && cp -f results/archive/n44_cands_20k.txt ./n44_cands_20k.txt && sbatch cluster/archive/deploy/gpu_spike.sh'
```
...
production levers multiply). Driver `cluster/archive/deploy/gpu_spike.sh` (30min, --gres=gpu:1,
...
verdict/node cross-check). Candidate file shipped at `results/archive/n44_cands_20k.txt`.
...
`docs/research/n44_search_narrowing_research.md` (measured-dead block); direct encoding: do not
...
pre-07-24 history → docs/archive/handoff/HANDOFF_ARCHIVE_to_2026-07-23.md (~3,100 lines out of the per-session context
...
VALIDATED ON REAL DATA.** Full ranked program: `docs/research/n44_search_narrowing_research.md`
...
`docs/research/2026-07-28-per-arm-candidate-resume-design.md` + the 07-28 late
...
checkpoint; spec at `docs/research/2026-07-28-per-arm-candidate-resume-design.md`);
