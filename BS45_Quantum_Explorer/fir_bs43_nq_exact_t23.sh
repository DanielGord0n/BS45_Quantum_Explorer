...
#SBATCH --job-name=BS43_t23_fir_nq
#SBATCH --output=bs43_t23_fir_nq_output_%A_%a.txt
#SBATCH --array=0-9
...
# Fir searches combo quarter [16777216,25165824) (Nibi-quarter BACKUP while Nibi is in maintenance) of the 33554432 first-4-layer combos (WZ_SPLIT=4).
...
LO=$TASK_LO
HI=$TASK_HI
export WZ_CKPT=$SCRATCH/bs45/ckpt_fir_nq_${SLURM_ARRAY_TASK_ID}.txt
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ] && [ "$CHAIN" -lt "$MAXCHAIN" ]; then
  ...
  PEND_OTHER=$(squeue -h -u dangord -n BS43_t23_fir_nq -t PD -o '%i' 2>/dev/null | grep -vc "^${SLURM_ARRAY_JOB_ID}_")
  ALL_DONE=1
  for t in $(seq 0 $((NTASKS-1))); do
    TLO=$(( CLUSTER_LO + t * SPAN )); THI=$(( TLO + SPAN ))
    [ $THI -gt $CLUSTER_HI ] && THI=$CLUSTER_HI
    CK=$SCRATCH/bs45/ckpt_fir_nq_$t.txt
  ...
  if [ "$ALL_DONE" -eq 0 ] && [ "${PEND_OTHER:-0}" -eq 0 ]; then
    sbatch --export=ALL,CHAIN=$((CHAIN+1)) --dependency=afterany:$SLURM_ARRAY_JOB_ID fir_bs43_nq_exact_t23.sh \
...
echo "=============================================="
echo "  BS(43,42) wz_exact_t23 — Fir (Nibi-quarter backup)"
