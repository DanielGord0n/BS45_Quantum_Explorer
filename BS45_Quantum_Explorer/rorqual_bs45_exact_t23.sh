...
#SBATCH --job-name=BS45_t23_rorqual
#SBATCH --output=bs45_t23_rorqual_output_%A_%a.txt
#SBATCH --array=0-9
...
LO=$TASK_LO
HI=$TASK_HI
export WZ_CKPT=$SCRATCH/bs45/ckpt_bs45_rorqual_${SLURM_ARRAY_TASK_ID}.txt
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ] && [ "$CHAIN" -lt "$MAXCHAIN" ]; then
  PEND_OTHER=$(squeue -h -u dangord -n BS45_t23_rorqual -t PD -o '%i' 2>/dev/null | grep -vc "^${SLURM_ARRAY_JOB_ID}_")
  ALL_DONE=1
  for t in $(seq 0 $((NTASKS-1))); do
    TLO=$(( CLUSTER_LO + t * SPAN )); THI=$(( TLO + SPAN ))
    [ $THI -gt $CLUSTER_HI ] && THI=$CLUSTER_HI
    CK=$SCRATCH/bs45/ckpt_bs45_rorqual_$t.txt
    if [ ! -f "$CK" ] || [ "$(cat "$CK" 2>/dev/null || echo 0)" -lt "$THI" ]; then ALL_DONE=0; break; fi
  done
  if [ "$ALL_DONE" -eq 0 ] && [ "${PEND_OTHER:-0}" -eq 0 ]; then
    sbatch --export=ALL,CHAIN=$((CHAIN+1)) --dependency=afterany:$SLURM_ARRAY_JOB_ID rorqual_bs45_exact_t23.sh \
...
echo "=============================================="
echo "  BS(45,44) WORLD-RECORD attempt — rorqual"
