...
#SBATCH --job-name=BS43_t23_nibi
#SBATCH --output=bs43_t23_nibi_output_%A_%a.txt
#SBATCH --array=0-9
...
LO=$TASK_LO
HI=$TASK_HI
export WZ_CKPT=$SCRATCH/bs45/ckpt_nibi_${SLURM_ARRAY_TASK_ID}.txt
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ] && [ "$CHAIN" -lt "$MAXCHAIN" ]; then
  ...
  PEND_OTHER=$(squeue -h -u dangord -n BS43_t23_nibi -t PD -o '%i' 2>/dev/null | grep -vc "^${SLURM_ARRAY_JOB_ID}_")
  ALL_DONE=1
  for t in $(seq 0 $((NTASKS-1))); do
    TLO=$(( CLUSTER_LO + t * SPAN )); THI=$(( TLO + SPAN ))
    [ $THI -gt $CLUSTER_HI ] && THI=$CLUSTER_HI
    CK=$SCRATCH/bs45/ckpt_nibi_$t.txt
    if [ ! -f "$CK" ] || [ "$(cat "$CK" 2>/dev/null || echo 0)" -lt "$THI" ]; then ALL_DONE=0; break; fi
  done
  if [ "$ALL_DONE" -eq 0 ] && [ "${PEND_OTHER:-0}" -eq 0 ]; then
    sbatch --export=ALL,CHAIN=$((CHAIN+1)) --dependency=afterany:$SLURM_ARRAY_JOB_ID nibi_bs43_exact_t23.sh \
...
echo "=============================================="
echo "  BS(43,42) wz_exact_t23 — Nibi"
