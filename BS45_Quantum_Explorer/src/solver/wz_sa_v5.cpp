/*
 * Wang-Zhu BS Solver v5 — Joint SA over Wang-Zhu Pair Manifold
 * with Cross-Thread Champion Sharing + Adaptive Escape
 * CP493 - Directed Research - Daniel Gordon
 *
 * Improvements over v4 (which plateaus at cost 16-32 on BS(43) after 20h):
 *   1. Global champion state shared across all threads on a node (mutex-protected).
 *      Stuck threads pull from g_champion_state and perturb, dramatically
 *      improving cooperation across the 192-core node.
 *   2. 4-pair sampled descent (200K samples) when cost <= 12 — escapes the
 *      cost-8/16 basins that 3-pair can't.
 *   3. Adaptive kick escalation: small kick first (k=3), then medium (k=6),
 *      then large (k=12), then full random restart. Tracks consecutive
 *      kick failures to scale aggression.
 *   4. Pull-from-global-champion: every Nth stall, copy the global champion
 *      and perturb it with k=4..8 instead of always kicking from local best.
 *   5. Longer SA chains: iterations 150K, restarts 6.
 *   6. Sampled 3-pair descent budget doubled to 100K samples.
 *
 * State = (A[n+1], B[n+1], C[n], D[n]) all encoded as Wang-Zhu mirror-pairs.
 * Cost  = 5*sum-mismatch + sum_{s=1..ms-1} |NPAF[s]|.
 *
 * Usage:  ./wz_sa_v5 <n> [seed_offset]
 *
 * macOS:  g++ -O3 -std=c++17 -Xpreprocessor -fopenmp \
 *           -I/opt/homebrew/opt/libomp/include -L/opt/homebrew/opt/libomp/lib -lomp \
 *           -o wz_sa_v5 src/solver/wz_sa_v5.cpp
 * Cluster: g++ -O3 -march=native -std=c++17 -fopenmp -o wz_sa_v5 src/solver/wz_sa_v5.cpp
 */
...
// =====================================================================
//  Signature enumeration (identical to v4)
// =====================================================================
...
// Global champion state (cross-thread sharing)
...
// Sampled 3-pair descent (100K samples in v5, was 50K in v4).
...
// NEW in v5: Sampled 4-pair descent. Only invoked when cost is small (<= 12)
// since this is the depth we need to escape cost-8/16 basins.
static bool four_pair_sampled_descent(State &st, int &cur_cost, int n1, int n, int ms,
                                        const Sig &sig, mt19937 &rng) {
  int ab_slots = (n1 + 1) / 2;
  int cd_slots = (n  + 1) / 2;
  int total = ab_slots + cd_slots;
  if (total < 4) return cur_cost == 0;

  bool improved = true;
  int delta1[128], delta2[128], delta3[128], delta4[128];
  int oldv1[8], oldv2[8], oldv3[8], oldv4[8];
  int combos_i[16][4], combos_j[16][4], combos_k[16][4], combos_l[16][4];
  uniform_int_distribution<> slot_dist(0, total - 1);
  ...
  while (improved && cur_cost > 0) {
    ...
    for (int sample = 0; sample < MAX_SAMPLES && !improved; sample++) {
      ...
      int s2; do { s2 = slot_dist(rng); } while (s2 == s1);
      int s3; do { s3 = slot_dist(rng); } while (s3 == s1 || s3 == s2);
      int s4; do { s4 = slot_dist(rng); } while (s4 == s1 || s4 == s2 || s4 == s3);

      bool i_AB = (s1 < ab_slots), j_AB = (s2 < ab_slots);
      bool k_AB = (s3 < ab_slots), l_AB = (s4 < ab_slots);
      int li = i_AB ? s1 : s1 - ab_slots;
      int lj = j_AB ? s2 : s2 - ab_slots;
      int lk = k_AB ? s3 : s3 - ab_slots;
      int ll = l_AB ? s4 : s4 - ab_slots;

      int nci = enumerate_slot(li, n1, n, i_AB, combos_i);
      int ncj = enumerate_slot(lj, n1, n, j_AB, combos_j);
      int nck = enumerate_slot(lk, n1, n, k_AB, combos_k);
      int ncl = enumerate_slot(ll, n1, n, l_AB, combos_l);

      int ci = uniform_int_distribution<>(0, nci - 1)(rng);
      int cj = uniform_int_distribution<>(0, ncj - 1)(rng);
      int ck = uniform_int_distribution<>(0, nck - 1)(rng);
      int cl = uniform_int_distribution<>(0, ncl - 1)(rng);
      ...
      if (!apply_forced(st, n1, n, ms, ll, l_AB, combos_l[cl], delta4, oldv4)) {
        revert_forced(st, n1, n, ms, lk, k_AB, oldv3, delta3);
        revert_forced(st, n1, n, ms, lj, j_AB, oldv2, delta2);
        revert_forced(st, n1, n, ms, li, i_AB, oldv1, delta1);
        continue;
      ...
      if (nc < cur_cost) {
        ...
        revert_forced(st, n1, n, ms, ll, l_AB, oldv4, delta4);
        revert_forced(st, n1, n, ms, lk, k_AB, oldv3, delta3);
        revert_forced(st, n1, n, ms, lj, j_AB, oldv2, delta2);
        revert_forced(st, n1, n, ms, li, i_AB, oldv1, delta1);
      }
...
// v5 intensify chain: 1 -> 2 -> 1 -> 3 -> 2 -> 1 -> 4 (if cost low) -> 3 -> 2 -> 1
static bool intensify(State &st, int &cur_cost, int n1, int n, int ms, const Sig &sig, mt19937 &rng) {
  ...
  if (cur_cost <= 24 && two_pair_descent(st, cur_cost, n1, n, ms, sig)) return true;
  if (one_pair_descent(st, cur_cost, n1, n, ms, sig)) return true;
  if (cur_cost <= 12 && four_pair_sampled_descent(st, cur_cost, n1, n, ms, sig, rng)) return true;
  if (cur_cost <= 24 && three_pair_sampled_descent(st, cur_cost, n1, n, ms, sig, rng)) return true;
  if (cur_cost <= 16 && two_pair_descent(st, cur_cost, n1, n, ms, sig)) return true;
  if (one_pair_descent(st, cur_cost, n1, n, ms, sig)) return true;
  return cur_cost == 0;
}
...
// =====================================================================
//  v5 SA parameters — longer chains, more restarts
// =====================================================================
struct SAParams {
  ...
  int    restarts         = 6;
  int    reheat_threshold = 12000;
  double reheat_ratio     = 0.6;
  int    kick_after_stall = 4000;
};

// =====================================================================
//  Per-signature joint SA with cross-thread champion sharing
// =====================================================================
static bool solve_joint_SA(int n, int n1, int ms, const Sig &sig,
  ...
  // Track consecutive kick failures for adaptive kick escalation.
  ...
  for (int restart = 0; restart < sa.restarts; restart++) {
    ...
    if (restart > 0) {
      // Restart strategies based on which restart we're on:
      //  restart 1-2: kick from local best (k = total_slots/4)
      //  restart 3-4: pull from global champion if available (k = total_slots/3)
      //  restart 5+:  full random restart
      if (restart >= 5) {
        state_random_init(curr, n1, n, ms, rng);
        cur_cost = curr.cost(sig, ms);
      } else if (restart >= 3) {
        // Pull from global champion if it's for the same sig and is good.
        bool pulled = false;
        {
    ...
    // Post-restart deterministic descent.
    {
      State work = curr;
      int wc = cur_cost;
    ...
    for (int iter = 0; iter < sa.iterations; iter++) {
      ...
      if (stall_for_kick > sa.kick_after_stall) {
        // Try aggressive intensification from best_state before kicking.
        ...
        // Adaptive kick: escalate based on consecutive failures.
        State source;
        bool used_champion = false;

        // Every 3rd failure, try pulling from global champion (cross-thread cooperation).
        if (consecutive_kick_failures >= 2 && (consecutive_kick_failures % 3) == 0) {
          lock_guard<mutex> lk(g_champion_mutex);
          if (g_champion_valid &&
              g_champion_sig.a == sig.a && g_champion_sig.b == sig.b &&
              g_champion_sig.c == sig.c && g_champion_sig.d == sig.d) {
        ...
        // Kick size escalates with consecutive failures.
        int k;
        if (consecutive_kick_failures < 3) {
          k = 2 + uniform_int_distribution<>(0, 3)(rng);
        } else if (consecutive_kick_failures < 6) {
          k = 5 + uniform_int_distribution<>(0, 4)(rng);
        } else if (consecutive_kick_failures < 10) {
          k = max(8, total_slots / 4);
        } else {
          // Truly stuck — full random restart for this thread.
          state_random_init(curr, n1, n, ms, rng);
          cur_cost = curr.cost(sig, ms);
        ...
        kopt_kick(curr, n1, n, ms, k, rng);
        cur_cost = curr.cost(sig, ms);
        // Hotter restart when pulling champion (more diversification needed).
        temp = used_champion ? sa.initial_temp * 0.8 : sa.initial_temp * 0.6;
        stall_for_kick = 0;
        continue;
      }
...
int main(int argc, char **argv) {
  ...
  cout << "========================================================" << endl;
  cout << "  BS(" << n1 << "," << n << ") v5 Solver — Joint Wang-Zhu SA" << endl;
  cout << "  with Cross-Thread Champion Sharing + Adaptive Escape" << endl;
...
#pragma omp parallel
  {
    ...
    while (!g_found.load(memory_order_relaxed)) {
      // ε-greedy signature selection biased toward sigs with low best-cost history.
      int si = -1;
      for (int tries = 0; tries < 16; tries++) {
        ...
        // Skip probabilistically based on fail count; bias toward low sig_best.
        double p = 1.0;
        if (fails >= 3) p = 1.0 / (1.0 + 0.4 * (fails - 2));
        // Boost probability for sigs that have shown low cost.
        if (sb != INT_MAX && sb <= 16) p = min(1.0, p * 2.0);
        if (uniform_real_distribution<>(0.0, 1.0)(rng) < p + 0.15) {
          si = cand;
          break;
        }
      }
      if (si < 0) si = uniform_int_distribution<>(0, (int)sigs.size() - 1)(rng);
