...
#SBATCH --job-name=BS45_t23_trilli
#SBATCH --output=bs45_t23_trillium_output_%A_%a.txt
#SBATCH --array=0-9
#SBATCH --account=def-ikotsire
#SBATCH --mail-type=END,FAIL

# === BS(45,44) WORLD-RECORD ATTEMPT — wz_exact_t23, Trillium ===
...
# Trillium searches combo quarter [25165824,33554432) of the 33554432 first-4-layer combos.
...
LO=$TASK_LO
HI=$TASK_HI
export WZ_CKPT=$SCRATCH/bs45/ckpt_bs45_trillium_${SLURM_ARRAY_TASK_ID}.txt
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ] && [ "$CHAIN" -lt "$MAXCHAIN" ]; then
  PEND_OTHER=$(squeue -h -u dangord -n BS45_t23_trilli -t PD -o '%i' 2>/dev/null | grep -vc "^${SLURM_ARRAY_JOB_ID}_")
  ALL_DONE=1
...
echo "=============================================="
echo "  BS(45,44) WORLD-RECORD attempt — trillium"
